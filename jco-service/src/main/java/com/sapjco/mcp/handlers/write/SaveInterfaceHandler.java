...
/**
 * Handler for SaveInterface tool.
 * Saves ABAP interface source code via atomic lock/save/unlock operation.
 */
@Slf4j
@Component
public class SaveInterfaceHandler extends AbstractWriteHandler {
    ...
    public Tool getToolDefinition() {
        return ToolSchemaBuilder.builder()
                ...
                .requiredString("interface_name", "Name of the ABAP interface")
                .optionalString("source_code", "Complete ABAP interface source code (provide this OR source_file)")
                .optionalString("source_file",
                        "Path to file containing ABAP source code (provide this OR source_code). " +
                        "Use file paths returned by GetInterface for easy edit-save workflows.")
                ...
                .buildTool(
                        "SaveInterface",
                        "Save ABAP interface source code via JCo. REQUIRED: Must provide session_id from CreateSession. " +
    ...
    public CallToolResult handle(McpSyncServerExchange exchange, CallToolRequest request) {
        ...
        validation = validateRequired(args, "interface_name");
        if (validation != null) return validation;
        ...
        log.info("SaveInterface called: {} (session: {}, transport: {}, source: {})",
                interfaceName, sessionId, transportNumber != null ? transportNumber : "(auto)",
        ...
        try {
            ...
            String rawResponse = atomicSaveObject(sessionId, interfaceName, "interface",
                    effectiveSourceCode, transportNumber);

            return formatSaveResponse(resolved, rawResponse);

        } catch (Exception e) {
            return handleSaveError(interfaceName, e);
        }
    }
}
