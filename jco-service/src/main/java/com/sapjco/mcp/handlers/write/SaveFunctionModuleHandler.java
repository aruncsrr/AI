...
/**
 * Handler for SaveFunctionModule tool.
 * Saves ABAP function module source code via atomic lock/save/unlock operation.
 * Locks the function module directly (not the parent function group) to ensure
 * the lock handle matches the resource being saved.
 */
@Slf4j
@Component
public class SaveFunctionModuleHandler extends AbstractWriteHandler {
    ...
    public Tool getToolDefinition() {
        return ToolSchemaBuilder.builder()
                ...
                .requiredString("group_name", "Name of the ABAP function group containing the function module")
                .requiredString("fmodule_name", "Name of the ABAP function module")
                .optionalString("source_code", "Complete ABAP function module source code (provide this OR source_file)")
                .optionalString("source_file",
                        "Path to file containing ABAP source code (provide this OR source_code). " +
                        "Use file paths returned by GetFunctionModule for easy edit-save workflows.")
                ...
                .buildTool(
                        "SaveFunctionModule",
                        "Save ABAP function module source code via JCo. REQUIRED: Must provide session_id from CreateSession. " +
                        "JCo is required for proper SAP session state management. Automatically handles atomic " +
                        "lock/save/unlock operations (locks the function module directly). " +
                        "Provide source via source_code (inline) OR source_file (file path)."
                );
    }
    ...
    public CallToolResult handle(McpSyncServerExchange exchange, CallToolRequest request) {
        ...
        log.info("SaveFunctionModule called: {}.{} (session: {}, transport: {}, source: {})",
                groupName, moduleName, sessionId, transportNumber != null ? transportNumber : "(auto)",
        ...
        try {
            ...
            // Execute atomic save operation (locks function group, saves function module)
            String rawResponse = atomicSaveFunctionModule(sessionId, groupName, moduleName,
                    effectiveSourceCode, transportNumber);

            return formatSaveResponse(resolved, rawResponse);

        } catch (Exception e) {
            return handleSaveError(groupName + "." + moduleName, e);
        }
    }
}
