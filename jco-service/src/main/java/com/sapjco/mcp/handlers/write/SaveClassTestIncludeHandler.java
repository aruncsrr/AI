...
/**
 * Handler for SaveClassTestInclude tool.
 * Saves ABAP class test include source code via atomic lock/save/unlock operation.
 */
@Slf4j
@Component
public class SaveClassTestIncludeHandler extends AbstractWriteHandler {
    ...
    public Tool getToolDefinition() {
        return ToolSchemaBuilder.builder()
                ...
                .optionalString("source_code", "Complete ABAP test class source code (provide this OR source_file)")
                .optionalString("source_file",
                        "Path to file containing ABAP source code (provide this OR source_code). " +
                        "Use file paths returned by GetClassInclude(include_type=testClasses) for easy edit-save workflows.")
                ...
                .buildTool(
                        "SaveClassTestInclude",
                        "Save ABAP class test include source code via JCo. REQUIRED: Must provide session_id from CreateSession. " +
                        "Saves to the testClasses include of the specified class. Automatically handles atomic " +
    ...
    public CallToolResult handle(McpSyncServerExchange exchange, CallToolRequest request) {
        ...
        log.info("SaveClassTestInclude called: {} (session: {}, transport: {}, source: {})",
                className, sessionId, transportNumber != null ? transportNumber : "(auto)",
        ...
        try {
            ...
            String rawResponse = atomicSaveClassTestInclude(sessionId, className,
                    effectiveSourceCode, transportNumber);

            return formatSaveResponse(resolved, rawResponse);

        } catch (Exception e) {
            log.error("SaveClassTestInclude failed for {}", className, e);
