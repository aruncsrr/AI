...
/**
 * Formatter for GetTable tool responses.
 * Converts raw table XML into human-readable table definition.
 */
@Slf4j
@Component
public class TableFormatter extends AbstractADTFormatter {
    ...
    public String format(String rawResponse) throws FormattingException {
        ...
        return formatTableDefinition(rawResponse, null, "active");
    }

    /**
     * Format table response with context parameters.
     *
     * @param rawResponse The raw XML response
     * @param tableName The table name (for header display)
     * @param version The version (active/inactive)
     * @return Formatted string
     * @throws FormattingException if formatting fails
     */
    public String formatWithContext(String rawResponse, String tableName, String version) throws FormattingException {
        if (rawResponse == null || rawResponse.trim().isEmpty()) {
            throw FormattingException.emptyResponse(getToolName());
        }

        return formatTableDefinition(rawResponse, tableName, version);
    }

    private String formatTableDefinition(String xml, String tableName, String version) {
        ...
        String displayName = tableName != null ? tableName : (String) rootData.getOrDefault("name", "TABLE");

        StringBuilder sb = new StringBuilder();
        sb.append(String.format("Table: %s (version: %s)\n", displayName.toUpperCase(), version));
        ...
        if (!type.isEmpty()) {
            String typeLabel = formatTableType(type);
        ...
        sb.append("\nNote: Use PreviewTableData to view table contents, or GetDiscovery to find\n");
        sb.append("      the technical settings endpoint for field definitions.\n");

        return sb.toString();
    }

    private String formatTableType(String type) {
        ...
        switch (type) {
            ...
            case "TABL/VW": return "Database View";
            default: return type;
        }
    }
