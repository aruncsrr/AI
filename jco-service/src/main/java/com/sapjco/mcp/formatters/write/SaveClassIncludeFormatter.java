...
/**
 * Formatter for SaveClassInclude tool responses.
 * Handles save results for class includes (definitions, implementations, macros, testClasses).
 * SAP typically returns empty body on success, or error details on failure.
 */
@Slf4j
@Component
public class SaveClassIncludeFormatter extends AbstractADTFormatter {

    @Override
    public String getToolName() {
        return "SaveClassInclude";
    }

    @Override
    public String format(String rawResponse) throws FormattingException {
        StringBuilder sb = new StringBuilder();
        sb.append("Save Class Include Result\n");
        sb.append(boxDivider(60)).append("\n\n");

        if (rawResponse == null || rawResponse.trim().isEmpty()) {
            sb.append("Status: SUCCESS\n\n");
            sb.append("The class include was saved successfully.\n");
            sb.append("Note: The include is saved but NOT activated.\n");
