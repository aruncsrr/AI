...
/**
 * Formatter for SaveClassTestInclude tool responses.
 * Handles save results for class test includes (unit test classes).
 * SAP typically returns empty body on success, or error details on failure.
 */
@Slf4j
@Component
public class SaveClassTestIncludeFormatter extends AbstractADTFormatter {

    @Override
    public String getToolName() {
        return "SaveClassTestInclude";
    }

    @Override
    public String format(String rawResponse) throws FormattingException {
        StringBuilder sb = new StringBuilder();
        sb.append("Save Class Test Include Result\n");
        sb.append(boxDivider(60)).append("\n\n");

        if (rawResponse == null || rawResponse.trim().isEmpty()) {
            sb.append("Status: SUCCESS\n\n");
            sb.append("The test include was saved successfully.\n");
            sb.append("Note: The test include is saved but NOT activated.\n");
