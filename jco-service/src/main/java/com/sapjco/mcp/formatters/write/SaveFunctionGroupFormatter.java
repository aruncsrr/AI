...
/**
 * Formatter for SaveFunctionGroup tool responses.
 * SAP typically returns empty body on success, or error details on failure.
 */
@Slf4j
@Component
public class SaveFunctionGroupFormatter extends AbstractADTFormatter {

    @Override
    public String getToolName() {
        return "SaveFunctionGroup";
    }

    @Override
    public String format(String rawResponse) throws FormattingException {
        StringBuilder sb = new StringBuilder();
        sb.append("Save Function Group Result\n");
        sb.append(boxDivider(60)).append("\n\n");

        if (rawResponse == null || rawResponse.trim().isEmpty()) {
            sb.append("Status: SUCCESS\n\n");
            sb.append("The function group was saved successfully.\n");
            sb.append("Note: The function group is saved but NOT activated.\n");
