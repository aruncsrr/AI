...
class GetIncludeHandlerTest extends BaseHandlerTest {

    @InjectMocks
    private GetIncludeHandler handler;

    private JcoSession mockTempSession;

    private static final Path TEST_FILE_PATH = Path.of("/tmp/sap-mcp/dev_100/include/ZTEST_INCLUDE.abap");
    ...
    @Test
    void getInclude_success() throws Exception {
        setupTempSession();
        setupExecuteInContext(INCLUDE_SOURCE);
    ...
    @Test
    void getInclude_withInactiveVersion() throws Exception {
        setupTempSession();
        setupExecuteInContext(INCLUDE_SOURCE);
    ...
    @Test
    void getInclude_missingName_returnsError() {
    ...
    @Test
    void getInclude_notFound_returnsError() throws Exception {
        setupTempSession();
        when(jcoSessionManager.executeInContext(eq(TEST_TEMP_SESSION_ID), any()))
                .thenThrow(new RuntimeException("Include not found"));

        CallToolRequest request = requestBuilder()
                .arg("include_name", "ZNONEXISTENT")
                .build();
    ...
    @Test
    void getInclude_withFunctionGroupName_success() throws Exception {
        setupTempSession();
        setupExecuteInContext(INCLUDE_SOURCE);
    ...
    @Test
    void getInclude_withFunctionGroupName_missingName_returnsError() {
        CallToolRequest request = requestBuilder()
                .arg("function_group_name", "ZFG_TEST")
                .build();
